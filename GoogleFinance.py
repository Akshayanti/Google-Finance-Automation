...
class GoogleFinance:
    ...
    def check_date(self, input_date):
        ...
        if re.compile("\d{2}-\d{2}-\d{4}").match(input_date):
            try:
                ...
                pass
        logging.log(level=logging.ERROR, msg="The date should be in format DD-MM-YYYY")
        exit(1)
    ...
    def read_dir(self, dirname):
        ...
        """
        logging.log(level=logging.INFO, msg="Reading contents of {} directory for CSV and TSV files".format(dirname))
        for item in os.listdir(dirname):
            if os.path.isfile(os.path.join(dirname, item)) and item.endswith(".tsv"):
                self.read_tsv_file(item)
            elif os.path.isfile(os.path.join(dirname, item)) and item.endswith(".csv"):
                self.read_porfolio(item)
        logging.log(level=logging.INFO, msg="Finished reading contents of directory")
            ...
            if action == "BUY":
                try:
                    ...
                    message = "Could not add {x} units at {y}/unit on {z} for {s}".format(x=qty, y=price, z=d8,
                                                                                          s=symbol)
                    logging.log(logging.ERROR, msg=message)
                    self.driver.get(self.portfolios[group])
            elif action == "SELL":
